...
import type { BoltEventDetails } from '../types/data'

const boltChecklistStorageKey = 'georgia-travel-2026:kutaisi-bolt:checklist'

interface BoltEventContentProps {
  details: BoltEventDetails
}
...
function BoltSection({ icon, title, children }: BoltSectionProps) {
  return (
...
export function BoltEventContent({ details }: BoltEventContentProps) {
  const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>(
    () => {
      try {
        const savedValue = localStorage.getItem(boltChecklistStorageKey)
  ...
  useEffect(() => {
    localStorage.setItem(boltChecklistStorageKey, JSON.stringify(checkedItems))
  }, [checkedItems])
      ...
      <div className="lg:col-span-2">
        ...
        >
          <div className="mx-auto max-w-2xl">
            {details.orderSteps.map((step, index) => (
              <div key={step} className="relative flex gap-4 pb-4 last:pb-0">
                {index < details.orderSteps.length - 1 && (
        ...
        >
          <div className="rounded-[1.5rem] bg-stone-50 p-5 sm:p-7">
            ...
            <ul className="mt-5 grid gap-3 sm:grid-cols-2">
              {details.vehicleChecks.map((item, index) => (
                <li
                  key={item}
                  className={`flex gap-3 rounded-2xl bg-white p-4 text-sm leading-6 shadow-sm ${
                    index === details.vehicleChecks.length - 1
                      ? 'text-red-800 sm:col-span-2'
                      : 'text-stone-600'
                  }`}
                >
                  <Check
